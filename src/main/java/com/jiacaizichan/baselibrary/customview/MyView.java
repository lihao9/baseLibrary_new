...
//        bitmap.compress(Bitmap.CompressFormat.JPEG, 100, bos);
    ...
    public String saceToBase64(boolean clearBlank,int blank){
        String result = null;
        Bitmap bitmap = cachebBitmap;
        //BitmapUtil.createScaledBitmapByHeight(srcBitmap, 300);//  压缩图片
        if (clearBlank) {
...
//                result = Base64.encodeToString(buffer,Base64.DEFAULT);
                result = "data:image/png;base64,"+android.util.Base64.encodeToString(buffer,0,buffer.length, android.util.Base64.DEFAULT);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
